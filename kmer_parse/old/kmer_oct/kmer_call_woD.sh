...
do
  ...
  cp ${fl1} ./$fl/
  while read line; do ~divyae/jellyfish-2.2.3/bin/jellyfish count -s 100000 -m $i -C -o ./$fl/${line%.fna}_mercount.jf $line; done < FN_singles.txt
  ...
  for x in *mercount.jf #pass indivdually as x - for all the files that end with "k"_mercount.jf
    ...
    printf "${y}\n" >> ${x%jf}tsv #dump the desired output in this file
  done
